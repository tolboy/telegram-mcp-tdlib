...
| P1 | **Out-of-band or host approval for destructive tools**: add an approval artifact that a tool-call argument cannot self-assert, initially for `leave_chat`, `ban_user`, `delete_message`, `join_chat_by_link`, and `register_internal_chat` | High | Medium | Implemented as opt-in `MCP_DESTRUCTIVE_APPROVAL=auto`: host-native elicitation where negotiated, otherwise a single-use nonce-protected IPv4-loopback page. Both answers return outside the model's turn. The shared dispatch gate runs ahead of the handler, keys off destructive classification rather than `requiresConfirmation`, refuses expired/invalid answers, and can be pinned to `elicitation` or `loopback` for fail-closed deployments. |
