...
| P1 | **Out-of-band or host approval for destructive tools**: add an approval artifact that a tool-call argument cannot self-assert, initially for `leave_chat`, `ban_user`, `delete_message`, `join_chat_by_link`, and `register_internal_chat` | High | Medium | Implemented as opt-in `MCP_DESTRUCTIVE_APPROVAL=elicitation`, using the host-native path the MCP spec already defines. The answer returns over the protocol rather than through the model's turn, so it cannot be self-asserted. The gate runs at the shared dispatch ahead of the handler, keys off the destructive classification rather than `requiresConfirmation`, and refuses clients that do not advertise the capability instead of downgrading. A local out-of-band token remains the option for clients that never gain elicitation; it is not built, because a token the model must carry in its arguments reintroduces the problem unless bound and single-use. |
