...
class SetForumTopicsEnabledToolTest {
    ...
    private lateinit var tool: SetForumTopicsEnabledTool
    ...
    fun setUp() {
        ...
        tool = SetForumTopicsEnabledTool(
    ...
    fun `definition returns correct tool name`() {
        assertEquals("set_forum_topics_enabled", tool.definition().name())
    }

    @Test
    fun `enables forum topics by default`() {
        ...
        verify { telegramClient.setForumTopicsEnabled(42L, true, true) }
    }

    @Test
    fun `passes explicit disabled state`() {
        every { entityResolver.resolve(42 as Any) } returns 42L
        ...
        assertFalse(result.isError)
        verify { telegramClient.setForumTopicsEnabled(42L, false, false) }
    }
