...
class ArchiveChatToolTest {
    ...
    private lateinit var tool: ArchiveChatTool
    ...
    fun `definition returns correct tool name`() {
        assertEquals("archive_chat", tool.definition().name())
    }

    @Test
    fun `archives chat successfully`() {
        every { entityResolver.resolve(42 as Any) } returns 42L
        every { telegramClient.archiveChat(42L) } returns true

        val result = tool.execute(exchange, mapOf("chat_id" to 42))

        assertFalse(result.isError)
        verify { operationGuardService.checkPermission("archive_chat", any()) }
