...
class SaveDraftToolTest {
    ...
    private lateinit var tool: SaveDraftTool
    ...
    fun `definition returns correct tool name`() {
        assertEquals("save_draft", tool.definition().name())
    }

    @Test
    fun `saves draft successfully`() {
        ...
        verify { operationGuardService.checkPermission("save_draft", any()) }
        verify { guardrailService.validateChatAccess(42L) }
        verify { guardrailService.validateInput("Hello draft") }
    }

    @Test
    fun `saves draft with reply`() {
        every { entityResolver.resolve(42 as Any) } returns 42L
