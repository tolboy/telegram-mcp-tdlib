...
class SendReactionToolTest {
    ...
    private lateinit var tool: SendReactionTool
    ...
    fun `definition returns correct tool name`() {
        assertEquals("send_reaction", tool.definition().name())
    }

    @Test
    fun `adds reaction successfully`() {
        every { entityResolver.resolve(42 as Any) } returns 42L
        every { telegramClient.addReaction(42L, 100L, "👍", false) } returns true
        ...
        verify { operationGuardService.checkPermission("send_reaction", any()) }
        verify { guardrailService.validateChatAccess(42L) }
    }

    @Test
    fun `adds big reaction with is_big flag`() {
        every { entityResolver.resolve(42 as Any) } returns 42L
        every { telegramClient.addReaction(42L, 100L, "❤️", true) } returns true

        val result = tool.execute(
            exchange,
            mapOf("chat_id" to 42, "message_id" to 100, "emoji" to "❤️", "is_big" to true),
        )

        assertFalse(result.isError)
        verify { telegramClient.addReaction(42L, 100L, "❤️", true) }
    }
