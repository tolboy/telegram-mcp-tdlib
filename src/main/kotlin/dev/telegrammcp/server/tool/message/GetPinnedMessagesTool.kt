...
class GetPinnedMessagesTool(
    ...
    private val log = StructuredLogger.forClass<GetPinnedMessagesTool>()

    companion object {
        const val TOOL_NAME = "get_pinned_messages"
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Get all pinned messages in a Telegram chat", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Getting pinned messages for chat {}", chatId)

            guardrailService.validateChatAccess(chatId)
            val messages = telegramClient.getPinnedMessages(chatId)
            ...
            log.withTool(TOOL_NAME).error("Failed to get pinned messages: {}", ex.message, ex)
