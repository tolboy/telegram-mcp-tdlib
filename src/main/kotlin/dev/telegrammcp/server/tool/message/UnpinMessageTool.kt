...
class UnpinMessageTool(
    ...
    private val log = StructuredLogger.forClass<UnpinMessageTool>()

    companion object {
        const val TOOL_NAME = "unpin_message"
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Unpin a message in a Telegram chat", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Unpinning message {} in chat {}", messageId, chatId)

            guardrailService.validateChatAccess(chatId)
            telegramClient.unpinMessage(chatId, messageId)

            auditService.record(TOOL_NAME, arguments, AuditOutcome.SUCCESS)
            val json = objectMapper.writeValueAsString(mapOf("unpinned" to true, "message_id" to messageId, "chat_id" to chatId))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to unpin message: {}", ex.message, ex)
