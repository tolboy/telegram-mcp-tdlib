...
class PinMessageTool(
    ...
    private val log = StructuredLogger.forClass<PinMessageTool>()

    companion object {
        const val TOOL_NAME = "pin_message"
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Pin a message in a Telegram chat", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            val messageId = (arguments["message_id"] as? Number)?.toLong()
                ?: throw InvalidToolInputException("message_id is required")
            val disableNotification = arguments["disable_notification"]?.toString()?.toBoolean() ?: false

            log.withTool(TOOL_NAME).info("Pinning message {} in chat {}", messageId, chatId)

            guardrailService.validateChatAccess(chatId)
            telegramClient.pinMessage(chatId, messageId, disableNotification)

            auditService.record(TOOL_NAME, arguments, AuditOutcome.SUCCESS)
            val json = objectMapper.writeValueAsString(mapOf("pinned" to true, "message_id" to messageId, "chat_id" to chatId))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to pin message: {}", ex.message, ex)
