 ...
 *
 * Removes a previously placed reaction from a message.
 */
@Component
class RemoveReactionTool(
    ...
    private val log = StructuredLogger.forClass<RemoveReactionTool>()

    companion object {
        const val TOOL_NAME = "remove_reaction"
        ...
        private val INPUT_SCHEMA = """
        {
          ...
          "properties": {
            ...
            "emoji": { "type": "string", "description": "Emoji of the reaction to remove" }
          },
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Remove an emoji reaction previously placed on a message", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Removing reaction from message {} in chat {}", messageId, chatId)

            telegramClient.removeReaction(chatId, messageId, emoji)
            auditService.record(TOOL_NAME, arguments, AuditOutcome.SUCCESS)

            val json = objectMapper.writeValueAsString(
                mapOf("chat_id" to chatId, "message_id" to messageId, "emoji" to emoji, "removed" to true),
            )
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to remove reaction: {}", ex.message, ex)
