...
/**
 * MCP tool: **clear_draft** — removes any persisted draft on a chat.
 */
@Component
class ClearDraftTool(
    ...
    private val log = StructuredLogger.forClass<ClearDraftTool>()

    companion object {
        const val TOOL_NAME = "clear_draft"
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Clear the draft message on a chat", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Clearing draft on chat {}", chatId)
            telegramClient.clearDraft(chatId)
            auditService.record(TOOL_NAME, arguments, AuditOutcome.SUCCESS)

            val json = objectMapper.writeValueAsString(mapOf("chat_id" to chatId, "cleared" to true))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to clear draft: {}", ex.message, ex)
