...
class DemoteAdminTool(
    ...
    private val log = StructuredLogger.forClass<DemoteAdminTool>()

    companion object {
        const val TOOL_NAME = "demote_admin"
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Demote an admin to regular member in a Telegram group/channel", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Demoting admin {} in chat {}", userId, chatId)

            guardrailService.validateChatAccess(chatId)
            telegramClient.setChatMemberAdmin(chatId, userId, isAdmin = false)

            auditService.record(TOOL_NAME, arguments, AuditOutcome.SUCCESS)
            val json = objectMapper.writeValueAsString(mapOf("demoted" to true, "user_id" to userId, "chat_id" to chatId))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to demote admin: {}", ex.message, ex)
