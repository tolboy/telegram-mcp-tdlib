 ...
 * MCP tool: **ban_user**
 *
 * Bans a user from a group/supergroup/channel. Destructive, requires confirmation.
 */
@Component
class BanUserTool(
    ...
    private val log = StructuredLogger.forClass<BanUserTool>()

    companion object {
        const val TOOL_NAME = "ban_user"
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Ban a user from a Telegram group/channel (destructive, requires confirmation)", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Banning user {} from chat {}", userId, chatId)
            ...
            val json = objectMapper.writeValueAsString(mapOf("banned" to true, "user_id" to userId, "chat_id" to chatId))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to ban user: {}", ex.message, ex)
