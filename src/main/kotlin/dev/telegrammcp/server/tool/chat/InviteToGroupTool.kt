...
/**
 * MCP tool: **invite_to_group**
 *
 * Adds one or more users to an existing group/supergroup.
 */
@Component
class InviteToGroupTool(
    ...
    private val log = StructuredLogger.forClass<InviteToGroupTool>()

    companion object {
        const val TOOL_NAME = "invite_to_group"
        ...
        private val INPUT_SCHEMA = """
        {
          ...
          "properties": {
            "chat_id": {
              "type": ["string", "number"],
              "description": "Chat identifier of the group"
            },
            "user_ids": {
              "type": "array",
              "items": { "type": "number" },
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Invite users to an existing Telegram group or supergroup", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Inviting {} users to chat {}", userIds.size, chatId)

            guardrailService.validateChatAccess(chatId)
            telegramClient.addChatMembers(chatId, userIds)

            auditService.record(TOOL_NAME, arguments, AuditOutcome.SUCCESS)
            val json = objectMapper.writeValueAsString(mapOf("invited" to userIds.size, "chat_id" to chatId))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to invite users: {}", ex.message, ex)
