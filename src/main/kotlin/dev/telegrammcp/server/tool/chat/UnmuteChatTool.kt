...
/**
 * MCP tool: **unmute_chat**
 *
 * Unmutes notifications for a chat.
 */
@Component
class UnmuteChatTool(
    ...
    private val log = StructuredLogger.forClass<UnmuteChatTool>()

    companion object {
        const val TOOL_NAME = "unmute_chat"
        ...
        private val INPUT_SCHEMA = """
        {
          ...
          "properties": {
            "chat_id": {
              "type": ["string", "number"],
              "description": "Chat identifier to unmute"
            }
          },
          "required": ["chat_id"]
        }
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Unmute notifications for a Telegram chat", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Unmuting chat {}", chatId)
            guardrailService.validateChatAccess(chatId)
            telegramClient.unmuteChat(chatId)

            auditService.record(TOOL_NAME, arguments, AuditOutcome.SUCCESS)
            val json = objectMapper.writeValueAsString(mapOf("unmuted" to true, "chat_id" to chatId))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to unmute chat: {}", ex.message, ex)
