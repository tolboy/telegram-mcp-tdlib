 ...
 * MCP tool: **unban_user**
 *
 * Unbans a previously banned user from a group/supergroup/channel.
 */
@Component
class UnbanUserTool(
    ...
    private val log = StructuredLogger.forClass<UnbanUserTool>()

    companion object {
        const val TOOL_NAME = "unban_user"
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Unban a user from a Telegram group/channel", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Unbanning user {} from chat {}", userId, chatId)
            ...
            val json = objectMapper.writeValueAsString(mapOf("unbanned" to true, "user_id" to userId, "chat_id" to chatId))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to unban user: {}", ex.message, ex)
