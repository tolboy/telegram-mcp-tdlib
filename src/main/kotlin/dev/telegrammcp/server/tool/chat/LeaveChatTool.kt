 ...
 * MCP tool: **leave_chat**
 *
 * Leaves a chat. This is a destructive operation requiring confirmation.
 */
@Component
class LeaveChatTool(
    ...
    private val log = StructuredLogger.forClass<LeaveChatTool>()

    companion object {
        const val TOOL_NAME = "leave_chat"
        ...
        private val INPUT_SCHEMA = """
        {
          ...
          "properties": {
            "chat_id": {
              "type": ["string", "number"],
              "description": "Chat identifier to leave"
            },
            "confirmed": {
              "type": "boolean",
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Leave a Telegram chat/group/channel (destructive, requires confirmation)", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Leaving chat {}", chatId)
            ...
            val json = objectMapper.writeValueAsString(mapOf("left" to true, "chat_id" to chatId))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to leave chat: {}", ex.message, ex)
