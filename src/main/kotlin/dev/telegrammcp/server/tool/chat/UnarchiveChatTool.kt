...
/**
 * MCP tool: **unarchive_chat**
 *
 * Moves a chat from archive back to the main list.
 */
@Component
class UnarchiveChatTool(
    ...
    private val log = StructuredLogger.forClass<UnarchiveChatTool>()

    companion object {
        const val TOOL_NAME = "unarchive_chat"
        ...
        private val INPUT_SCHEMA = """
        {
          ...
          "properties": {
            "chat_id": {
              "type": ["string", "number"],
              "description": "Chat identifier to unarchive"
            }
          },
          "required": ["chat_id"]
        }
    ...
    override fun definition(): McpSchema.Tool =
        dev.telegrammcp.server.tool.ToolSupport.definition(TOOL_NAME, "Unarchive a Telegram chat", INPUT_SCHEMA, objectMapper)
    ...
    override fun execute(
        ...
        return try {
            ...
            log.withTool(TOOL_NAME).info("Unarchiving chat {}", chatId)
            guardrailService.validateChatAccess(chatId)
            telegramClient.unarchiveChat(chatId)

            auditService.record(TOOL_NAME, arguments, AuditOutcome.SUCCESS)
            val json = objectMapper.writeValueAsString(mapOf("unarchived" to true, "chat_id" to chatId))
            dev.telegrammcp.server.tool.ToolSupport.textResult(json)
        } catch (ex: Exception) {
            log.withTool(TOOL_NAME).error("Failed to unarchive chat: {}", ex.message, ex)
